...
CoreManager* g_core_manager = nullptr;
...
// Core Manager class implementation =============================================================================
pthread_t CoreManager::create_thread(int core_id, bool is_simulation) {
    pthread_t tid;
...
void CoreManager::stop_thread(pthread_t tid) {
    // 查找线程的上下文
...
CoreManager::CoreManager(int sim_per_core, int val_per_core, int max_threads) : 
...
void CoreManager::initialize(int num_cores, const std::vector<int>& core_ids) {
...
void CoreManager::initialize_with_config(int num_cores, int sim_threads, int val_threads, const std::vector<int>& core_ids) {
    // 临时保存原配置
...
int CoreManager::get_core_count() {
    std::lock_guard<std::mutex> lock(core_mutex);
    return active_cores.size();
}

std::pair<int, int> CoreManager::get_threads_per_core() {
    std::lock_guard<std::mutex> lock(core_mutex);
    return {sim_threads_per_core, val_threads_per_core};
}

int CoreManager::get_max_threads() {
    return max_available_threads;
}

int CoreManager::add_validation_thread(int core_id) {
    std::lock_guard<std::mutex> lock(core_mutex);
...
int CoreManager::add_core(int core_id) {
    std::lock_guard<std::mutex> lock(core_mutex);
...
int CoreManager::remove_core(int core_id) {
    std::lock_guard<std::mutex> lock(core_mutex);
...
int CoreManager::adjust_thread(int d_sim, int d_val) {
    std::lock_guard<std::mutex> lock(core_mutex);
...
std::vector<int> CoreManager::GetActiveCores() {
    std::lock_guard<std::mutex> lock(core_mutex);
    return active_cores;
}
// Core Manager class implementation =============================================================================
