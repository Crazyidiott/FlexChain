...
def plot_comparison_data(csv_files, output_dir='plots'):
    ...
    plt.plot(df1['elapsed_seconds'], df1['cpu_utilization'], 
             label='rl_agent - CPU utilization', color='#1f77b4', linestyle='-', alpha=0.8)
    plt.plot(df1['elapsed_seconds'], df1['memory_utilization'] * 100, 
             label='rl_agent - memory utilization', color='#ff7f0e', linestyle='-', alpha=0.8)
    
    # Static Configuration资源利用率 (红色系)
    plt.plot(df2['elapsed_seconds'], df2['cpu_utilization'], 
             label='static configuration - CPU utilization', color='#d62728', linestyle='--', alpha=0.8)
    plt.plot(df2['elapsed_seconds'], df2['memory_utilization'] * 100, 
             label='static configuration - memory utilization', color='#2ca02c', linestyle='--', alpha=0.8)
    
    plt.title('Resource Utilization Comparison: RL Agent vs Static Configuration')
    plt.xlabel('time(s)')
    plt.ylabel('utilization (%)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    # 保存图表
    output_file = os.path.join(output_dir, 'comparison_resources_plot.png')
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"对比资源图表已保存: {output_file}")
    plt.show()
