...
# 修改state从(84, 84)的游戏帧改为(5,)的状态参数

# eth
# Transition_dtype = np.dtype([('timestep', np.int32), ('state', np.uint8, (12,)), ('action', np.int32), ('reward', np.float32), ('nonterminal', np.bool_)])
# blank_trans = (0, np.zeros((12,), dtype=np.uint8), 0, 0.0, False)

# btc and zcash
# Transition_dtype = np.dtype([('timestep', np.int32), ('state', np.uint8, (5,)), ('action', np.int32), ('reward', np.float32), ('nonterminal', np.bool_)])
...
class ReplayMemoryRandom():
    def __init__(self, args, capacity, Transition_dtype, blank_trans):
    ...
    def _get_transitions(self, idxs):
        ...
        transitions = self.data[transition_idxs % self.capacity]
    ...
    def sample(self, batch_size):
        valid_indices = np.arange(self.capacity if self.full else self.index)
        valid_indices = valid_indices[
            (valid_indices >= self.history) &
            (valid_indices < (self.capacity if self.full else self.index) - self.n)
        ]
        idxs = np.random.choice(valid_indices, batch_size, replace=False)
        ...
        return idxs, states, actions, R, next_states, nonterminals, weights
    ...
    def __next__(self):
        if self.current_idx == self.capacity:
            raise StopIteration
        transitions = self.data[np.arange(self.current_idx - self.history + 1, self.current_idx + 1)]
        transitions_firsts = transitions['timestep'] == 0
        blank_mask = np.zeros_like(transitions_firsts, dtype=np.bool_)
        for t in reversed(range(self.history - 1)):
            blank_mask[t] = np.logical_or(blank_mask[t + 1], transitions_firsts[t + 1])
        transitions[blank_mask] = self.blank_trans
        state = torch.tensor(np.copy(transitions['state']), dtype=torch.float32, device=self.device)
        self.current_idx += 1
        return state

    next = __next__

class ReplayMemoryPrioritized():
  def __init__(self, args, capacity,Transition_dtype,blank_trans):
    self.device = args.device
    self.capacity = capacity
    self.history = args.history_length
    ...
    self.transitions = SegmentTree(capacity, self.Transition_dtype, self.blank_trans)
...
def createReplayMemory(args, memory_capacity):
    ...
    if args.prior_mem:
        ...
        return ReplayMemoryRandom(args, memory_capacity, Transition_dtype, blank_trans)